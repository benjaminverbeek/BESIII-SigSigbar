...
// WHAT IS THIS? ^ should add my masses? /BV
//const double velc = 29.9792458;  tof_path unit in cm.
const double velc = 299.792458;   // tof path unit in mm   <--- WHAT IS THIS? /BV
typedef std::vector<int> Vint;
typedef std::vector<HepLorentzVector> Vp4;

int Ncut0,Ncut1,Ncut2,Ncut3,Ncut4,Ncut5,Ncut6;  // Defining cut-counters. OK. /BV
 ...
 /* 
  ...
  if ( nt1 ) m_tuple1 = nt1;
    ...
    if ( m_tuple1 )    {
      ...
      status = m_tuple1->addItem ("rvxy0",  m_rvxy0); // what is this? /BV
      status = m_tuple1->addItem ("rvz0",   m_rvz0);  // Change to my particles?
  ...
  if ( nt2 ) m_tuple2 = nt2;
    ...
    if ( m_tuple2 )    {
      status = m_tuple2->addItem ("dthe",   m_dthe);  // Adds photon data? /BV
  ...
  status = m_tuple4->addItem ("mpip",   m_mpip);
  status = m_tuple4->addItem ("mpim",   m_mpim);
...
//    HepVector vv = m_reader.SigmaPrimaryVertex(runNo);
  ...
  cout << "Hello SigmaSigmabar world!" << endl; // edited!
...
//    if(fabs(Rxy) >= m_vr0cut) continue;
    ...
    if(fabs(Rvz0) >= 10.0) continue;  // where do these numbers come from? /BV 2022-11-22
    if(fabs(Rvxy0) >= 1.0) continue;  // 2022-11-23 EDITED OUT 2 LINES. Get more through? /Back - 2022-11-24
  ...
  if((nGood != 2)||(nCharge!=0)){
    ...
    // COMMENTED OUT 2022-11-23 15:56 FOR TEST /BV /Back 2022-11-24
  Ncut1++;
  Vint iGam;
  iGam.clear();
...
//      ptrk = ptrk.boost(-0.011,0,0);//boost to cms
  ...
  int npip = ipip.size();
  int npim = ipim.size();
  if(npip*npim != 1) return SUCCESS;  // What is this for? /BV 2022-11-22
...
//    double ecms = 3.097;
  ...
  // Question: different combos give different things?
  ...
  m_mrho0 = prho0.m(); // <-- Added 2022-11-24 21:30 /BV
	m_chi1 = kmfit->chisq();
	m_tuple4->write();  // Här lagras allt till m_tuple4 som lagras i .root-filen
        Ncut4++;
      }
    }
  }
