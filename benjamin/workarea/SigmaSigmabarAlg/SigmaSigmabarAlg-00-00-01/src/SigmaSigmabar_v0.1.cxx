// This is v0.1, where nCharged is changed to 4, required IP-region is made larger
...
//    HepVector vv = m_reader.SigmaPrimaryVertex(runNo);
  ...
  cout << "Hello SigmaSigmabar world! v0.0.1" << endl; // edited!
...
//    if(fabs(Rxy) >= m_vr0cut) continue; 
  ...
  // THROW AWAY TRACKS NOT ORIGINATING FROM INTERACTION REGION  
    ...
    if(fabs(Rvxy0) >= 1.0) continue;  // TODO: Here? Angle?
...
//    pid->setMethod(pid->methodLikelihood());  //for Likelihood Method  
    ...
    // TODO-GUESS: pid->identify(pid->onlyPion() | pid->onlyKaon() | pid->onlyProton());
    //    pid->identify(pid->onlyPion()); // TODO: Identify protons, how? pid->onlyProton()?
  ...
  // GUESS-TODO:
  // m_prob_pid_p = pid->probProton(); // REMEMBER: also add in header & initialize vairable
    m_tuple11->write();

//  if(pid->probPion() < 0.001 || (pid->probPion() < pid->probKaon())) continue;
    if(pid->probPion() < 0.001) continue; // TODO: or proton?
...
// identify simply by checking relative momentum, or use the above? Not sure. Ask?
    ...
    if(mdcKalTrk->charge() >0) { // if positive, its pi+
...
//      ptrk = ptrk.boost(-0.011,0,0);//boost to cms
    ...
    // Add more for proton! TODO. Copy-paste and modify for proton/-bar.
    // TODO: Karin suggested to just look at relative momentum, and then know type from there?
  }
...
*/
  ...
  if(npip*npim != 1) return SUCCESS;  // Here we check if there is only one pion and one anti-pion. TODO: Add proton/-bar.
